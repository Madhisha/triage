...
class TriagePredictionService:
    ...
    def _to_model_input(self, X: Any):
        """Convert DataFrame-like input to NumPy to match training-time estimator input."""
        if hasattr(X, 'values'):
            return X.values
        return np.asarray(X)


    def _model_predict(self, X: Any):
        """Predict classes using model input format consistent with training."""
        return self.ml_model.predict(self._to_model_input(X))


    def _model_predict_proba(self, X: Any):
        """Predict probabilities using model input format consistent with training."""
        return self.ml_model.predict_proba(self._to_model_input(X))


    def _predict_proba_for_shap(self, X: Any):
        """SHAP callback wrapper that avoids sklearn feature-name warnings."""
        return self._model_predict_proba(X)
    ...
    def _generate_ml_explanation(self, patient_data: Dict, 
        ...
        prediction_label = self.class_labels.get(prediction_class, f"Unknown (Class {prediction_class})")
        explanation += f"**{prediction_label}** priority level.\n"
    ...
    def predict_with_explanation(self, patient_data: Dict) -> Dict[str, Any]:
        ...
        if rule_prediction == 1:
            ...
            if self.ml_model is not None:
                ...
                raw_ml_prediction = int(self._model_predict(ml_features)[0])

                # Normalize based on model's trained class labels.
                # Some models are trained on [1,2,3], others on [0,1,2].
                model_classes = list(getattr(self.ml_model, 'classes_', []))
                if raw_ml_prediction in self.class_labels:
                    final_prediction = raw_ml_prediction
                elif set(model_classes) == {0, 1, 2} and raw_ml_prediction in {0, 1, 2}:
                    final_prediction = raw_ml_prediction + 1
                elif raw_ml_prediction in {0, 1, 2}:
                ...
                try:
                    probabilities = self._model_predict_proba(ml_features)[0]
                    class_list = list(getattr(self.ml_model, 'classes_', []))

                    if class_list and len(class_list) == len(probabilities):
                        if raw_ml_prediction in class_list:
                            confidence = float(probabilities[class_list.index(raw_ml_prediction)])
                        elif final_prediction in class_list:
                            confidence = float(probabilities[class_list.index(final_prediction)])
                        elif 0 <= raw_ml_prediction < len(probabilities):
                            confidence = float(probabilities[raw_ml_prediction])
                        else:
                    ...
                    print(
                        f"   Raw ML prediction: {raw_ml_prediction}, "
                        f"Final class: {final_prediction}, Confidence: {confidence:.3f}"
                    )
                except Exception as e:
        ...
        result = {
            ...
            'prediction_label': self.class_labels.get(final_prediction, f'Unknown (Class {final_prediction})'),
